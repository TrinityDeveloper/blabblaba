...
class OnSave_PlayerScript : public PlayerScript
{
public:
    OnSave_PlayerScript() : PlayerScript("OnSave_PlayerScript") { }

    void OnSave(Player * player)
    {
		if (player->GetTotalPlayedTime() <= 6000)
		{
			...
			case CLASS_HUNTER:
                player->learnSpell(674, false); // Dual Wield.
